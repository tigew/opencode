...
Detection is based on the model id matching `/gemma[-_.]?4/i`. To opt out or customise, set `toolParser` explicitly on the provider in `opencode.json`:
  ...
  "provider": {
    "my-llama-cpp": {
      ...
      "options": {
        ...
        "toolParser": [{ "type": "gemma4" }, { "type": "json" }]
      }
    }
  }
}
